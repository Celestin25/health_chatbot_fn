...
const UpdateProduct = (property: productProps) => {
  ...
  const [description, setDescription] = useState<string>(
    property.productData.description
  );
  const [discount, setDiscount] = useState<number>(
    property.productData.discount
  );
  ...
  const [category, setCategory] = useState<string>(
    property.productData.category
  );
  const [quantity, setQuantity] = useState<number>(
    property.productData.quantity
  );
  const [expiringDate, setExpiringDate] = useState<string>(
    property.productData.expiringDate
  );
  ...
  const [updateProduct, { isLoading: uploadLoading, isError }] =
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const data = {
        ...
        id: property.productData.productId,
      };
    ...
    <div className="w-full h-full bg-black/20  fixed flex items-center top-0 justify-center left-0">
      <div className="w-1/2 flex flex-col max-h-[90vh] overflow-y-scroll gap-[10px] bg-white p-4 rounded-[12px]">
        <div className="w-full flex flex-row font-outfit items-center justify-between">
          ...
          <div
            className="text-[20px] font-[700] text-black cursor-pointer hover:scale-125 transition-all duration-300"
            onClick={() => property.setModal(false)}
          >
            X
          </div>
        </div>
        ...
        >
          <div className="grid grid-cols-2 gap-[10px] w-full">
            ...
            <div className="flex flex-col gap-[6px] w-full">
              <span className="text-[16px] font-[400] text-[#666666] font-outfit">
                {" "}
                Medecine Category
              </span>
              <select
                ...
                }  rounded-[12px] font-outfit outline-none placeholder:font-[400] w-full`}
                id=""
              >
            ...
            <Input
              ...
              placeholder="Enter Product Name"
              error={error}
              onChange={(value) => setExpiringDate(value)}
            />
          </div>
          <div className="w-full flex gap-[10px]">
            <label
              htmlFor="images"
              className="w-1/2 cursor-pointer hover:scale-95 transition-all duration-300 flex items-center justify-center p-3 rounded-[4px] bg-secondary"
            >
              <span className="text-white font-[600] font-outfit">
                Select Images +
              </span>
            </label>
            <input
            ...
            <button
              type="submit"
              className="w-1/2 p-3 bg-primary flex items-center justify-center text-white font-[600] text-[20px] rounded-[6px]"
            >
            ...
            return (
              <div className="w-full h-[20vh] relative">
                <div
                  ...
                  className="text-red-600 absolute right-0 top-0 font-[800] cursor-pointer"
                >
                  X
                </div>
                <img
